...
pub fn noise3D(comptime T: type, x: T, y: T, z: T) T {
    ...
    const x_i: u8 = @intFromFloat(@mod(x, 256.0));
    const y_i: u8 = @intFromFloat(@mod(y, 256.0));
    const z_i: u8 = @intFromFloat(@mod(z, 256.0));
    ...
    return lerp(w, lerp(v, lerp(u, grad3D(T, permutation[aa], x_r, y_r, z_r), grad3D(T, permutation[ba], x_r - 1.0, y_r, z_r)), lerp(u, grad3D(T, permutation[ab], x_r, y_r - 1.0, z_r), grad3D(T, permutation[bb], x_r - 1.0, y_r - 1.0, z_r))), lerp(v, lerp(u, grad3D(T, permutation[aa + 1], x_r, y_r, z_r - 1.0), grad3D(T, permutation[ba + 1], x_r - 1.0, y_r, z_r - 1.0)), lerp(u, grad3D(T, permutation[ab + 1], x_r, y_r - 1.0, z_r - 1.0), grad3D(T, permutation[bb + 1], x_r - 1.0, y_r - 1.0, z_r - 1.0))));
}
...
test "3D noise" {
    ...
    try expect(noise3D(f64, -4.20, 10, 6.0) == 0.14208000000000043);
}
